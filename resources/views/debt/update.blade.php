...
<div class="card mx-auto w-50 mt-4 border-success">
    <div class="card-body">
        ...
        <form method="POST" class="mt-4" action="{{ route('debt.update', $debt->id) }}" role="form" enctype="multipart/form-data">
            @csrf
            <div class="mb-2">
                ...
                <select class="form-select {{ $errors->has('debter_data') ? 'is-invalid' : '' }}" name="debter_data">
                    ...
                    <option data-fullname="{{ $debter->fullname }}" data-description="{{ $debter->description }}" value="{{ $debter->id }}" {{ $debt->debter_id == $debter->id ? 'selected' : '' }}>{{ $debter->fullname }}</option>
                    @endforeach
            ...
            <div class="mb-2">
                ...
                <input type="number" name="total" class="form-control {{ $errors->has('total') ? 'is-invalid' : '' }}"
                    value="{{ $debt->total }}" placeholder="" aria-label="Name" aria-describedby="invalidCheckTotal">
                @if($errors->has('total'))
            ...
            <div class="mb-2">
                ...
                <textarea type="text" name="remarks" rows="5"
                    ...
                    aria-label="Name" aria-describedby="invalidCheckRemarks">{{ $debt->remarks }}</textarea>
...
<script type="text/javascript">
    $(document).ready(function() {
        console.log("Update Debt Page!");
        $("[name='debter_data']").on("change", function() {
            console.log("On change debter_data")
        ...
        $("[name='debter_data']").trigger("change");
    });
</script>
